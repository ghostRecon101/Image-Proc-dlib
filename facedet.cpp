...
int main(int argc, char const *argv[])
{
	try
	{
        ...
        for(unsigned long i = 0; i < dets.size(); i++)
        {
            ...
            s2 = sqrt(s2);
            unsigned long upwidth = s + t + s1 + s2 + t1;
            upwidth /= 5;
            ...
            s2 = pow(r.x(), 2) + pow(r.y(), 2);
            s2 = sqrt(s2);
            unsigned long dowidth = s + t + s1 + s2 + t1;
            dowidth /= 5;
            if(upwidth > dowidth)
                cout<<"\nUpper lip is wider.";
            else if(dowidth > upwidth)
                cout<<"\nLower lip is wider.";
            else
                cout<<"\nEqual thickness of upper and lower lip.";
            
            unsigned long towidth = upwidth + dowidth;
            towidth /= 2;

            cout<<"\nThe width of the lip is: "<<towidth;
        }
        cin.get();
	}
	catch (exception &e)
